...
def generate_evaluation_report(model_name: str,
    ...
    report = classification_report(y[1:,:].cpu().to(torch.int),
...
if __name__ == "__main__":
    ...
    parser.add_argument('model', type=str, help=f'model type, supported:{supported_models}')
    parser.add_argument('checkpoint_dir', metavar='DIR', help='path to model checkpoint')
    parser.add_argument('test_dir', metavar='DIR', help='path to test dataset')
    parser.add_argument('save_dir', metavar='DIR', default=Path(__file__).parent.absolute(), help='path to save evaluation result')
    parser.add_argument('--with_clean', type=bool, default=False)
    args = parser.parse_args()

    generate_evaluation_report(args.model, args.checkpoint_dir, args.test_dir, args.save_dir, args.with_clean)