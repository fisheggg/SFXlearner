...
def main():
    ...
    if args.model == "baseline":
        ...
        model = SUPPORTED_MODELS[args.model](in_channels, train_set.settings["n_classes"])        
    else:
    ...
    logger = pl.loggers.TensorBoardLogger(pathlib.Path(__file__).parent.parent.joinpath("lightning_logs"), name=log_name)
    trainer = pl.Trainer.from_argparse_args(args, callbacks=[early_stopping_callback], logger=logger, benchmark=True)
